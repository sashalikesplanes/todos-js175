...
module.exports = class PgPersistence {
  ...
  async sortedTodoLists() {
    ...
    const results = await Promise.all([dbQuery(ALL_TODOLISTS, this.username),
                                       dbQuery(ALL_TODOS, this.username)]);

    const allLists = results[0].rows;
    const allTodos = results[1].rows;

    if (!allLists || !allTodos) return undefined;

    allLists.forEach(list => {
      list.todos = allTodos.filter(todo => todo.todolist_id === list.id);
    });

    // Avoid the complexity of sorting by completion with SQL
    return this._paritionTodoLists(allLists);
  }
